...
def fetch_offers(categories: List[str]) -> List[Offer]:
    ...
    for category in categories:
        ...
        for term in search_terms:
            ...
            print(f"  [Nike] term='{term}' url={resp.url}")
            resp.raise_for_status()

            soup = BeautifulSoup(resp.text, "html.parser")

            product_elements = soup.select(
                "div.product-card, div.product-grid__items div.product-card"
            )
            ...
            for el in product_elements[:30]:
                ...
                was_price_el = el.select_one(
                    ".product-price--was, .product-card__price--strikethrough"
                )
